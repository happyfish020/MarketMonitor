...
def build_daily_report_text(
    ...
    if action_hint:
        lines.append("")
        lines.append("🧭 行为裁决（Action Governance）")
        lines.append(f"- Action: {action_hint.get('action')}")
        lines.append(f"- Reason: {action_hint.get('reason')}")
    
        allowed = action_hint.get("allowed", {})
        if allowed:
            lines.append(
                f"- Allowed: ETF={allowed.get('etf')}, SingleStock={allowed.get('single_stock')}"
            )
    
        limits = action_hint.get("limits", {})
        if limits:
            lines.append(
                f"- Limits: max_units_today={limits.get('max_units_today')}, "
                f"max_units_total={limits.get('max_units_total')}"
            )
    
        conditions = action_hint.get("conditions", [])
        if conditions:
