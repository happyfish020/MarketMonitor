...
class DBOracleProvider:
    ...
    # ==================================================
    # unified series fetcher for ProviderRouter('db')
    # ==================================================
    def fetch_series_df(self, symbol: str, window: int = 120, method: str = "index") -> pd.DataFrame:
        """Return a standardized time series df for SymbolSeriesStore.

        Output columns: date(str YYYY-MM-DD), close(float), pct(float)

        Notes:
        - This function is intentionally located in DBOracleProvider, so the whole
          'db' provider route is backed by THIS class only (FROZEN rule).
        - It does not depend on PRE_CLOSE/CHG_PCT columns; pct is computed from close.
        """
        method_norm = (method or "").strip().lower()
        window = int(window) if window and int(window) > 0 else 120

        # calendar-day backoff to cover trading-day window without relying on a calendar table
        end_dt = pd.Timestamp.today().date()
        start_dt = (pd.Timestamp.today() - pd.Timedelta(days=window * 3)).strftime("%Y-%m-%d")
        end_dt_str = end_dt.strftime("%Y-%m-%d")

        if method_norm in ("index", "idx", "index_daily"):
            rows = self.query_index_closes(index_code=symbol, window_start=start_dt, trade_date=end_dt_str)
            if not rows:
            ...
            return df[["date", "close", "pct"]].tail(window).reset_index(drop=True)

        # Not supported here (ETF paused; stock series still uses DS-level db queries)
        return pd.DataFrame(columns=["date", "close", "pct"])
