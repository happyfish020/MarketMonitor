...
def _build_phase2_structures(
    ...
    # Phase-2: Observations（DRS + continuity）
    observations = factors_bound.get("observations")
    if not isinstance(observations, dict):
        observations = {}

    try:
        drs_observation = DRSObservation().build(inputs=structure_facts, asof=trade_date_str)
    ...
    # ★ Phase-2: ExecutionSummary（真正接入点）
    # 只读 factors / structure / observations，不影响 gate/structure
    ...
    print(factors_bound.keys())
    return factors_bound


def _make_gate_decision(snapshot: Dict[str, Any], slots: dict, factors: Dict[str, FactorResult]):
    decider = ASharesGateDecider()
    gate_decision = decider.decide(snapshot = snapshot, slots = slots, factors=factors)
...
def run_cn_ashare_daily111(trade_date: str | None = None, is_intraday: bool = False, refresh_mode: str = "auto") -> None:
    ...
    gate_decision, snapshot = _make_gate_decision(snapshot, slots=factors_bound, factors=factors)
...
def run_cn_ashare_daily(
    ...
    # ✅ Phase-3：只调用函数，不写逻辑
    factors_bound = _build_phase3_structure_distribution(
        trade_date=trade_date_str,
        factors=factors,
        factors_bound=factors_bound,
    )
