...
class CrowdingConcentrationExplainBlock(ReportBlockRendererBase):
    """
    UnifiedRisk V12 · Crowding/Concentration Explain（核心字段解释）

    设计原则：
    - 只读 context / doc_partial，不抛异常
    - 优先 slots["crowding_concentration"]，允许从 intraday_overlay / observations 兜底读取
    """

    block_alias = "crowding_concentration.explain"
    title = "核心字段解释（拥挤/集中度/参与度/分化）"
    ...
    def render(self, context: ReportContext, doc_partial: Dict[str, Any]) -> ReportBlock:
        ...
        if not isinstance(src, dict) or not src:
            warnings.append("empty:crowding_concentration")
            payload = "（未提供 crowding_concentration / intraday_overlay 数据：该区块仅用于占位）"
        ...
        if isinstance(adv_ratio, (int, float)):
            ar = float(adv_ratio)
            if 0 <= ar <= 1.0:
                lines.append(f"- adv_ratio: **{ar:.4f} ({ar*100.0:.2f}%)**")
            else:
                lines.append(f"- adv_ratio: **{ar:.2f}%**")
        if isinstance(top20, (int, float)):
            lines.append(f"- top20_amount_ratio (proxy_top20pct): **{top20:.3f}**")
            lines.append("  - NOTE: 这是**拥挤代理口径**（分母/样本空间不同），不要当作全市场 Top20 成交占比。全市场口径请看 `liquidity_quality.details.top20_ratio`。")
        ...
        if interp:
            ...
            if direction:
                ...
                if str(direction).lower() in ("diverged", "diverge", "mixed"):
                    lines.append("方向不一致：ETF/代理与对照在方向上不一致，容易造成执行摩擦上升（买在错误一侧/买到没跟上的那边）。")
                else:
                    lines.append("方向一致：风险资产方向更一致（仍需结合拥挤度与参与度）。")

            if participation:
                lines.append(f"\n**participation: {participation}**")
                if str(participation).lower() in ("weak", "low"):
                    lines.append("参与度弱：涨跌扩散不足，常见于“指数能稳住，但多数个股没跟上”的盘面；即使结构未坏，进攻胜率通常偏低。")
                else:
                    lines.append("参与度不弱：不是极端窄参与（红盘扩散还可以）。但若拥挤度高或 Execution=D1/D2，仍不鼓励进攻，只作结构解释。")
            ...
            if divergence:
                ...
                lines.append("偏离幅度：用于区分“方向有分歧但幅度不大”与“剧烈背离”。")
    ...
    def _pick_src(self, context: ReportContext, doc_partial: Dict[str, Any]) -> Dict[str, Any]:
        # 1) direct slot
        v = context.slots.get("crowding_concentration") or context.slots.get("etf_spot_sync")
        if isinstance(v, dict) and v:
            return v

        # 2) intraday overlay container (common pattern: overlay["crowding_concentration"] / ["crowding_concentration"])
        overlay = context.slots.get("intraday_overlay") or context.slots.get("intraday") or context.slots.get("overlay")
        if isinstance(overlay, dict) and overlay:
            for k in ("crowding_concentration", "crowding_concentration", "crowding_concentration", "crowding_concentration_raw"):
                vv = overlay.get(k)
        ...
        # 3) doc_partial
        v = doc_partial.get("crowding_concentration")
        if isinstance(v, dict) and v:
            return v
        ...
        if isinstance(obs, dict) and obs:
            for k in ("crowding_concentration", "crowding_concentration", "intraday_overlay", "overlay"):
                vv = obs.get(k)
                if isinstance(vv, dict) and vv:
                    return vv
