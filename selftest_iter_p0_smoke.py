...
# -----------------------------
# Replay smoke gate (P0-1)
# -----------------------------

def _resolve_existing_path(rel_or_abs: str, roots: List[Path]) -> Optional[Path]:
    if not rel_or_abs:
        return None
    p = Path(rel_or_abs)
    if p.is_absolute():
...
def _search_project_roots() -> List[Path]:
    here = Path(__file__).resolve().parent
    cwd = Path.cwd().resolve()
    roots: List[Path] = [cwd, here]
    roots.extend(list(cwd.parents)[:6])
    roots.extend(list(here.parents)[:6])
    # de-dup
    out: List[Path] = []
...
def _run_replay_smoke_gate(args: argparse.Namespace) -> None:
    """
    Run scripts/replay_smoke.py as a subprocess. Fail the selftest if smoke fails.

    Enable via:
      - CLI: --smoke
    Defaults can be provided by env:
      - UR_SMOKE_DB
      - UR_SMOKE_BLOCK_SPECS
    """
    roots = _search_project_roots()

    # resolve smoke script
    smoke_script = (args.smoke_script or "").strip()
    smoke_path = None
    if smoke_script:
        smoke_path = _resolve_existing_path(smoke_script, roots)
    else:
        smoke_path = (
            _resolve_existing_path("scripts/replay_smoke.py", roots)
            or _resolve_existing_path("replay_smoke.py", roots)
        )
    if smoke_path is None or not smoke_path.exists():
        raise FileNotFoundError("replay_smoke.py not found. Put it under scripts/ or provide --smoke-script.")

    # resolve db
    dbp = (args.smoke_db or os.getenv("UR_SMOKE_DB") or "data/persistent/unifiedrisk.db").strip()
    db_path = _resolve_existing_path(dbp, roots)
    if db_path is None or not db_path.exists():
        raise FileNotFoundError(f"smoke db not found: {dbp} (set --smoke-db or env UR_SMOKE_DB)")

    # resolve block specs
    bsp = (args.smoke_block_specs or os.getenv("UR_SMOKE_BLOCK_SPECS") or "config/report_blocks.yaml").strip()
    bs_path = _resolve_existing_path(bsp, roots)
    if bs_path is None or not bs_path.exists():
        raise FileNotFoundError(f"smoke block specs not found: {bsp} (set --smoke-block-specs or env UR_SMOKE_BLOCK_SPECS)")
    ...
    cmd = [
        ...
        str(bs_path),
        "--limit",
        str(int(getattr(args, "smoke_limit", 20))),
        "--scan-limit",
        str(int(getattr(args, "smoke_scan_limit", 200))),
    ]
    if getattr(args, "smoke_strict_rendered", False):
        cmd.append("--strict-rendered")

    print("")
    print("========== Replay Smoke Gate ==========")
    print("script   :", smoke_path)
    print("db       :", db_path)
    print("block    :", bs_path)
    print("limit    :", int(getattr(args, "smoke_limit", 20)))
    print("scan     :", int(getattr(args, "smoke_scan_limit", 200)))
    print("strict   :", bool(getattr(args, "smoke_strict_rendered", False)))
    print("--------------------------------------")

    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", errors="replace")
    out, err = p.communicate()

    # Always echo smoke output for CI traceability
    if out:
        print(out.rstrip())
    if err:
        # many scripts print to stdout; keep stderr too
        print(err.rstrip())
...
def main() -> None:
    ...
    ap.add_argument('--debug', action='store_true', help='enable debug prints for dump/spec resolving')
    ap.add_argument('--dumps-dir', action='append', default=None, help='extra dirs to search doc_*.json dumps (repeatable)')

    # Replay smoke regression gate (optional; recommended in CI)
    ap.add_argument('--smoke', action='store_true', help='run replay_smoke gate after cases pass')
    ap.add_argument('--smoke-db', type=str, default=None, help='sqlite db path for replay_smoke (default: env UR_SMOKE_DB or data/persistent/unifiedrisk.db)')
    ap.add_argument('--smoke-block-specs', type=str, default=None, help='block specs yaml for replay_smoke (default: env UR_SMOKE_BLOCK_SPECS or config/report_blocks.yaml)')
    ap.add_argument('--smoke-limit', type=int, default=20, help='number of recent COMPLETED runs to check (default 20)')
    ap.add_argument('--smoke-strict-rendered', action='store_true', help='include /rendered in diffs (default ignores /rendered)')
    ap.add_argument('--smoke-script', type=str, default=None, help='path to replay_smoke.py (default: scripts/replay_smoke.py or replay_smoke.py)')
    ap.add_argument('--smoke-scan-limit', type=int, default=200, help='how many recent runs to scan to find COMPLETED (default 200)')
    args = ap.parse_args()
